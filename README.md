...
- `SPLIT_LEVELS`                      - Список технических имен карт вашего мода/игры *(как в `LOCALPATH_...`)*. Когда будет переход на одну из этих карт первые, автосплиттер сделает сплит сегмента таймера;
